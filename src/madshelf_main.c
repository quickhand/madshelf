...
#define PLUGINS_DIR "/.madshelf/plugins/"
    ...
    if(filelistcount>0)
    {
        ...
        for(count=0;count<8&&(file = (char*)ecore_list_next(filelist));count++)
        {
            ...
            else
            {
                ...
                if(strlen(bookmeta->title)>45)
                {
                    tempfilename=(char *)calloc(strlen(bookmeta->title) + 3+1, sizeof(char));
                    strncpy(tempfilename,bookmeta->title,45);
                    tempfilename[45]='\0';
                    strcat(tempfilename,"...");
                    ewl_label_text_set(EWL_LABEL(curwidget),tempfilename);
                    free(tempfilename);
                    ...
                    ewl_label_text_set(EWL_LABEL(curwidget),bookmeta->title);
            }
            ...
            if(!ecore_file_is_dir(fileconcat))
            {
                ...
                curwidget = ewl_widget_name_find(tempname);
                if(bookmeta!=NULL && bookmeta->author!=NULL)
                    ewl_label_text_set(EWL_LABEL(curwidget),bookmeta->author);
                else
...
{	
    ...
    */
    ...
    plugindir=(char *)calloc(strlen(PLUGINS_DIR)+strlen(homedir)+1,sizeof(char));
    sprintf(plugindir,"%s%s",homedir,PLUGINS_DIR);
